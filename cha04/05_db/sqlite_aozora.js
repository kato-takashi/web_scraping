...
db.serialize(function(){
  //SQLを実行してテーブルを作成
  db.run("CREATE TABLE IF NOT EXISTS items("+"item_id INTEGER PRIMARY KEY, "+"author TEXT, title TEXT, body TEXT)");

  //挿入用のプリペアドステートメントを準備
  var ins_stmt = db.prepare('INSERT INTO items(author, title, body)' + 'VALUES(?, ?, ?)');
  //各HTMLファイルを処理
...
//作者の出現回数を調べる
console.log("集計結果：");
db.each("SELECT author, COUNT(author) as cnt " + "FROM items GROUP BY author " + "ORDER BY cnt DESC",
  function(err, row){
